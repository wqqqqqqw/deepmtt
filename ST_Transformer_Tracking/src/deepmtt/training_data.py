...
def my_hx(x):
    ...
    r[0] = np.arctan2(x[1],x[0])  #极坐标的极角
    r[1] = np.sqrt(np.square(x[0])+np.square(x[1]))  #极径
    return r

#Batch creating
def creat_batch3(min_dn,max_dn,pos_noise,vel_noise,BN):
    batch_size = int(100*BN)
    my_traj, my_obser, Tran_m, Traj_turn = trajectory_batch_generator(batch_size,data_len)
    Traj_r = my_traj
    Obser = my_obser
    ...
    return Traj_r, Obser, Traj_s, Traj_r-Traj_s, Traj_turn
