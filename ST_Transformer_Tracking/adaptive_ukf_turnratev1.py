...
from LMTT_deta_bidr_0818 import Network
from batchdata_derive3 import trajectory_batch_generator
...
def latest_checkpoint(explicit_path=None):
    ...
    if not candidates:
        raise FileNotFoundError("No checkpoint found in Save_Model/")
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("Save_Model/ST_Transformer_0418_iter_10000.pth"),
    ...
    parser.add_argument("--out", type=Path, default=Path("adaptive_ukf_validation.png"), help="Output figure path")
